...
def set_imax_ui():
    ...
    st.markdown(f"""
        ...
        /* Permanent LED Ceiling with Twinkle */
        .ceiling-lights {{
            position: fixed;
...
if not st.session_state.logged_in:
    ...
    set_imax_ui() # Theme toggle removed
    ...
    if st.button("Generate IRS Report 🚀") or search_query:
        if not search_query and not ready:
            st.error("⚠️ Filter selection incomplete.")
        else:
            results = []
