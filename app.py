...
def get_tmdb_session():
    ...
    adapter = requests.adapters.HTimport streamlit as st
from tmdbv3api import TMDb, Movie, TV, Discover, Trending, Search
from datetime import datetime
import requests
import pandas as pd
...
# --- 1. THE ULTIMATE FIX: CONNECTION LIMITING ---
# We use a global session and a "PoolManager" to strictly limit open files
if 'persistent_session' not in st.session_state:
    session = requests.Session()
    # Hard-limiting the pool size ensures we NEVER exceed the server's file limit
...
# --- 3. THEATER STYLING ---
st.set_page_config(page_title="IRS - ICU Final Rectification", layout="wide", page_icon="🎬")
...
if not st.session_state.logged_in:
    ...
    # --- 7. USER PORTAL ---
        ...
        def get_safe_details(m_id, type_str):
            # Using the persistent session to fetch details securely
            try:
            ...
            try:
                ...
                if results:
                    ...
                    for item in results:
                        ...
                        cast, ott_n, ott_l, trailer = get_safe_details(item.id, m_type)
                        if not ott_n: continue # Skip if not on OTT
                        ...
                        processed += 1
            except Exception as e: st.error(f"System overloaded. Please refresh.")TPAdapter(pool_connections=100, pool_maxsize=500)
    session.mount('https://', adapter)
    return session
