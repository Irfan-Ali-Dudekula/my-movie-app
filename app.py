...
# --- 1. CORE STABILIZATION (Stops OSError 24) ---
@st.cache_resource
def get_bulletproof_session():
    """Limits open files to prevent crashes and buffering"""
    session = requests.Session()
    # Increased backoff ensures deep data fetches (Plot/Cast) don't time out
    retries = Retry(total=5, backoff_factor=2, status_forcelist=[429, 500, 502, 503, 504])
    # pool_maxsize=20 provides enough 'pipes' for 12 movie details simultaneously
...
if not st.session_state.logged_in:
    ...
    if app_mode == "Admin Command Center":
        ...
        # --- FIX: ROBUST DATA FETCHING (Ensures Plot/Cast never show N/A) ---
        ...
        if st.button("Generate Recommendations 🚀") or search_query:
            ...
            try:
                if search_query:
                    ...
                    # Smart Fallback: Tries India-specific OTT, then falls back to general search
                ...
                if results:
                    ...
                    for item in results:
                        ...
                        with cols[processed % 3]:
                            ...
                            if trailer:
                                st.video(trailer) # Embedded YouTube Trailer
                            
                            if ott_l:
                                # GREEN PLAY BUTTON for direct OTT app redirection
