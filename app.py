...
# --- 1. SESSION INITIALIZATION (Prevents Startup Crashes) ---
...
# --- 2. BULLETPROOF TMDB CONNECTION (Fixes "Too many open files") ---
@st.cache_resource
def get_safe_session():
    session = requests.Session()
    # Prevents crashes by managing connection pooling
...
# --- 3. UI: THEATER STYLING WITH DUAL THEMES ---
st.set_page_config(page_title="IRFAN CINEMATIC UNIVERSE (ICU)", layout="wide")

def apply_styles():
    # FIXED: Direct link to your theater image for Light Theme
    light_img = "https://i.ibb.co/hR0V6Yx/theater-light.png" 
    dark_img = "https://images.unsplash.com/photo-1489599849927-2ee91cede3ba?q=80&w=2070"
    
    if st.session_state.theme == "Light":
        main_bg = f"url({light_img})"
        sidebar_bg = "linear-gradient(180deg, #FFFFFF 0%, #FFFDD0 100%)" # White & Cream
        text_color = "#000000"
        card_bg = "rgba(255, 255, 255, 0.85)"
        led_style = "box-shadow: 0 0 15px #00d4ff, 0 0 30px #00d4ff; border: 2px solid #00d4ff;"
    else:
        main_bg = f"url({dark_img})"
        sidebar_bg = "linear-gradient(180deg, #000000 0%, #2C2C2C 100%)" # Black & Gray
        text_color = "#ffffff"
...
def fetch_movie_details(m_id, type_str):
    try:
...
if not st.session_state.logged_in:
    ...
    if st.button("Generate Recommendations 🚀") or search_query:
        ...
        if search_query:
            ...
            # SMART FALLBACK: Tries India OTT first, then general titles
            ...
            if not results:
                st.info("Showing popular titles (Limited India OTT links found).")
        ...
        if results:
            ...
            for i, item in enumerate(results[:15]):
                # FIXED: Accesses actual title string to avoid memory address output
                m_title = getattr(item, 'title', getattr(item, 'name', 'Unknown Title'))
                m_id = getattr(item, 'id', None)
                if not m_id: continue

                plot, cast, ott_n, ott_l, trailer = fetch_movie_details(m_id, m_type)
                with cols[i % 3]:
