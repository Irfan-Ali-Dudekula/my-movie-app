...
# --- 2. BULLETPROOF SESSION (Prevents Buffering/Crashes) ---
@st.cache_resource
...
def apply_styles():
    ...
    st.markdown(f"""
        ...
        .movie-card {{ border: 1px solid #444; padding: 15px; border-radius: 10px; background: rgba(0, 0, 0, 0.85); margin-bottom: 20px; min-height: 620px; }}
        ...
        .ott-label {{ color: #00d4ff; font-weight: bold; font-size: 1.0em; margin-bottom: 5px; display: block; }}
        .rating-badge {{ background: #f5c518; color: #000; padding: 2px 8px; border-radius: 5px; font-weight: bold; margin-bottom: 5px; display: inline-block; }}
        h1, h2, h3, p, span, label, .stMarkdown {{ color: #ffffff !important; }}
        </style>
        """, unsafe_allow_html=True)

# --- 4. DATA FETCHING (IMDb, Bio, & OTT Verified) ---
...
def fetch_details(m_id):
    try:
        ...
        # RECTIFIED: Strict Visual Guard - skips movies without bios or posters
        if not plot or len(plot) < 15: return None, None, None, None, None, 0.0
        ...
        try:
            ...
            if results:
                ...
                for item in results:
                    ...
                    if not poster: continue # RECTIFIED: Skips black placeholders
                    ...
                    with cols[processed % 3]:
                        ...
                        # Added IMDb Rating & OTT Platform name clearly
