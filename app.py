...
# --- 1. CRITICAL INITIALIZATION (Prevents AttributeError Crashes) ---
    ...
    if st.button("Enter ICU"):
        if u_name:
            if u_name.lower() == "irfan":
                ...
                else: st.error("Invalid Key!"); st.stop()
        ...
        if st.button("Generate Recommendations 🚀") or search_query:
            ...
            try:
                if search_query:
                    ...
                    if not results:
                        st.info("Showing popular titles (No direct streaming links found for this combination).")
