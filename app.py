...
def set_bg():
    # Your specified background image and video URLs
...
if not st.session_state.logged_in:
    ...
    theme_choice = st.sidebar.radio("Theater Lighting", ["Dark", "Light"])
    set_bg() # Background persists for both modes
    ...
    if st.button("Generate Recommendations 🚀") or search_query:
        if not search_query and not ready:
            st.error("⚠️ ICU Protocol: Select Content, Language, and Mood first!")
        else:
            results = []
