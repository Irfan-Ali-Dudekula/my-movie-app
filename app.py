...
if not st.session_state.logged_in:
    ...
    if st.button("Enter ICU"):
        if u_name:
            if u_name.lower() == "irfan":
                ...
                else: st.error("Invalid Security Key!"); st.stop()
            else:
    ...
    if app_mode == "Admin Command Center":
        ...
        lang_map = {"Telugu": "te", "Hindi": "hi", "Tamil": "ta", "Malayalam": "ml", "Kannada": "kn", "English": "en", "Korean": "ko"}
        ...
        if st.button("Generate Recommendations 🚀") or search_query:
            ...
            try:
                if search_query:
                    # FIX: Validates objects to prevent the 'str' error
                    ...
                    p = {
                        ...
                        'include_adult': st.session_state.u_age >= 18
                    }
                    
                    # 1. Try Strict India OTT Search
                    ...
                    if not results:
                        st.info("Showing all popular titles for this mood (Limited OTT links found).")
                ...
                if results:
                    ...
                    processed = 0
                    for item in results:
                        if processed >= 75: break # Locked to capacity
                        
                        m_id = getattr(item, 'id', None)
                        if not m_id: continue

                        # UPDATED YEAR VALIDATION: Allows minor overlaps for better discovery
                        rd_str = getattr(item, 'release_date', getattr(item, 'first_air_date', ''))
                        item_year = int(rd_str.split('-')[0]) if rd_str else 0
                        
                        # Only skips if the result is far outside the requested era
                        if not search_query and item_year > 0:
                            if item_year < (s_year - 2) or item_year > (e_year + 2): 
                                continue
