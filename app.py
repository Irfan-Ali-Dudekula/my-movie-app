...
# --- 3. UI: THEATER STYLING WITH ROYAL IMAX THEME ---
st.set_page_config(page_title="IRFAN CINEMATIC UNIVERSE (ICU)", layout="wide")

def apply_styles():
    # FIXED: Royal IMAX Light Theme Background
    light_img = "https://i.ibb.co/LhbXyH3/royal-imax-light.png" 
    ...
    st.markdown(f"""
        ...
        .movie-card {{ {led_style} padding: 15px; border-radius: 10px; background: {card_bg}; margin-bottom: 20px; transition: 0.3s; min-height: 550px; }}
        ...
        h1, h2, h3, p, span, label, .stMarkdown, .stText {{ color: {text_color} !important; }}
        </style>
...
if not st.session_state.logged_in:
    ...
    # FIXED: Syntax error on line 85 (Unterminated string)
    lang_map = {
    ...
    if st.button("Generate Recommendations 🚀") or search_query:
        ...
        try:
            if search_query:
                ...
                # FIXED: Logic to handle TV Show vs Movie discovery parameters
            ...
            if results:
                ...
                for item in results:
                    ...
                    # FIXED: Rectified Output Mismatch
                    ...
                    if not plot or plot == "Loading...": continue
